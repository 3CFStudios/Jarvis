...
def run() -> None:
    ...
    chat_ui.set_status(f"Mode: {current_mode}")
    ...
    def handle_user_text(text: str) -> None:
        ...
        mode = resolve_mode_command(text)
        if mode is not None:
            current_mode = mode
        ...
        try:
            persona_description = str(personas.get(current_mode, {}).get("description", ""))
            plan = planner.build_plan(text, current_mode, persona_description)
            if plan.say.strip():
    ...
    logger.info("Jarvis started with mode=%s", current_mode)
    chat_ui.run()
